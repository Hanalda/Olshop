...
export default class Cart extends React.Component {
    ...
    getCarts = () => {
        let items = localStorage.getItem('cart') ? JSON.parse(localStorage.getItem
    ('cart')) : []
        let total = 0
        let num = 0
        items.forEach(item => {
    ...
    removeFromCart = (produk) => {
        ...
        let cart = carts.filter(item => item.id_produk !== produk.id_produk);
                        ...
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                {carts.map((produk, index) =>
                                    <tr ley={index}>
                                        <td>
                                            <h4 className="text-capitalize font-weight-bold">{produk.nama_produk}</h4>
                                            <h6 className="card-text"><small>price: </small>Rp.{produk.harga}</h6>
                                        </td>
                                        <td>
                                            <h5>
                                                <span className="badge badge-secondary">{produk.qty}</span>
                                            </h5>
                                        </td>
                                        <td>
                                            <h5>
